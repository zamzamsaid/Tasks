...
Insert into EMPLOYEES (EMPLOYEE_ID,FIRST_NAME,LAST_NAME,EMAIL,PHONE,HIRE_DATE,MANAGER_ID,JOB_TITLE) values (1,'Tommy','Bailey','tommy.bailey@example.com','515.123.4567','2016-06-17',null,'President'); /*one*/
Insert into EMPLOYEES (EMPLOYEE_ID,FIRST_NAME,LAST_NAME,EMAIL,PHONE,HIRE_DATE,MANAGER_ID,JOB_TITLE) values (3,'Blake','Cooper','blake.cooper@example.com','515.123.4569','2016-01-13',1,'Administration Vice President'); /*one*/
Insert into EMPLOYEES (EMPLOYEE_ID,FIRST_NAME,LAST_NAME,EMAIL,PHONE,HIRE_DATE,MANAGER_ID,JOB_TITLE) values (2,'Jude','Rivera','jude.rivera@example.com','515.123.4568','2016-09-21',1,'Administration Vice President'); /*one*/
...
Insert into EMPLOYEES (EMPLOYEE_ID,FIRST_NAME,LAST_NAME,EMAIL,PHONE,HIRE_DATE,MANAGER_ID,JOB_TITLE) values (14,'Elliot','Brooks','elliot.brooks@example.com','515.124.4567','2016-12-07',9,'Accountant');
...
Insert into ORDERS (ORDER_ID,CUSTOMER_ID,STATUS,SALESMAN_ID,ORDER_DATE) values (65,6,'Shipped',64,'2013-06-21');
...
-- 1. count the number of customer 
SELECT COUNT(*) AS NO_CUSTOMER
FROM customers;
-- 2. Sum of All Orders' Quantities FROM ORDER ITEM BECAUSE QUANTITY COL IN IT
SELECT * FROM sales.order_items;
SELECT SUM(quantity)
FROM order_items;
-- SUM QUANTITY WHICH LESS THAN 5000 AND ORDER DATE GRATER OR EQUAL  IT IS DATE 2017-01-01
...
SELECT status , SUM(quantity * unit_price) AS "TOTAL SALES"
FROM order_items OI
JOIN orders O 
ON (OI.order_id = O.order_id)
GROUP BY status
...
SELECT country_name,COUNT(warehouse_id) AS Total_Warehouses
FROM warehouses W
JOIN locations L
...
SELECT category_name ,COUNT(*) AS "TOTAL PRODUCTS"
FROM products P
...
FROM order_items;
---
-- how to select he manger and the emplyees work under this manager
...
-- to check if above code work correct or not
select count(*) from employees where manager_id = 2;

-----
-- 1. List All Orders with Customer Information
SELECT o.order_id,c.name AS customer_name,o.order_date,o.status, C.website,C.address,c.credit_limit
FROM orders o
JOIN customers c 
...
SELECT e.employee_id,e.first_name,e.last_name, e.job_title, e.email, e.phone,m.first_name as Manger
FROM employees e
JOIN employees m 
...
select cu.name,c.first_name, c.email,c.phone
from customers cu
join contacts c
using(customer_id);
...
SELECT l.city, l.state, COUNT(w.warehouse_id) AS total_warehouses
FROM locations l
...
SELECT o.order_id, o.status, o.order_date,c.name AS customer_name,
...
SELECT r.region_id, r.region_name, COUNT(c.country_id) AS total_countries
FROM regions r
LEFT JOIN countries c ON r.region_id = c.region_id
GROUP BY r.region_id, r.region_name

-- use INNER JOIN clause to retrieve data from the orders and order_items
-- use the INNER JOIN with USING clause to retrieve data from orders and order_items
-- use the INNER JOIN with USING clause to retrieve customer_name, order_id, order_date, item_id, quantity,  unit_price
-- use the INNER JOIN with USING clause to retrieve customer_name, order_id, order_date, item_id, product_name, quantity,  unit_price
-- retrieve order_id, status, first_name, last_name from both orders and employees using LEFT JOIN.
-- use LEFT JOIN clauses to join three tables: orders, employees and customers and retrieve order_id, customer_name, status,first_name,last_name
-- get the order_id, status, employee_id, last_name of order 58.
-- get the employee_id, last_name, first_name, order_id, status of the salesman id 57 using RIGHT JOIN.
-- Calculate the averages of standard costs and list prices.
-- calculate the average DISTINCT list prices.
-- calculate the average list price of products by category.
-- retrieve the average of list price, category_name for each category.
-- return the average of list price, product categories whose average list prices are greater than 1000 by sorting categories names in Ascending order.
-- retrieve category names and the number of products in each. In addition, return only category whose number of products is greater than 50.
