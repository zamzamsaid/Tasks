...
def run_etl():
    ...
    def transform_data(df, store_id):
        ...
        df['Total_Price_OMR'] = df.apply(
            lambda row: row['Total_Price'] * 0.385 if row['CurrencyType'] == 'USD' else row['Total_Price'],
            axis=1
        )
    ...
    try:
        ...
        # Insert Customers (only CustomerID now)
        ...
        # Insert Stores (only StoreID now)
...
def run_etl():
    ...
    def transform_data(df, store_id):
        ...
        if 'CurrencyType' in df.columns:
            ...
            df['CurrencyType'] = 'USD'  # default to USD if missing

        # Calculate total price
        df['Total_Price'] = df['Qty'] * df['Unit_Price']

        # Convert only if currency is USD
        df['Total_Price_OMR'] = df.apply(
            lambda row: row['Total_Price'] * 0.385 if row['CurrencyType'] == 'USD' else row['Total_Price'],
            axis=1
        )
